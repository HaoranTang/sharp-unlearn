...
from corruptions import GaussianNoiseTransform, GlassBlurTransform, SnowTransform


def cifar10_dataloaders_no_val(
    batch_size=128, data_dir="datasets/cifar10", num_workers=12
):
...
def cifar100_dataloaders(
    ...
    if corruptions == "gaussian":
        corrupt = GaussianNoiseTransform(severity=3)
    elif corruptions == "glassblur":
        corrupt = GlassBlurTransform(severity=3)
    elif corruptions == "snow":
        corrupt = SnowTransform(severity=3)
    else:
        corrupt = transforms.Lambda(lambda x: x)
    if no_aug:
        train_transform = transforms.Compose(
            [   
                corrupt,
        ...
        train_transform = transforms.Compose(
            [
                ...
                transforms.RandomCrop(32, padding=4),
                transforms.RandomHorizontalFlip(),
                transforms.RandAugment(magnitude=15, num_ops=2), # vit override to reduce overfitting
                transforms.ToTensor(),
            ]
        )
        print("Data augmentation = randomcrop(32,4) + randomhorizontalflip")
...
def cifar100_dataloaders_no_val(
    batch_size=128, data_dir="datasets/cifar100", num_workers=12
):
    train_transform = transforms.Compose(
        [
            transforms.RandomCrop(32, padding=4),
...
class TinyImageNet:
    ...
    def __init__(self, args, normalize=True):
        ...
        self.train_path = os.path.join("/scratch/gilbreth/thr/dev/RUM/datasets/tiny-imagenet-200/train")
        self.val_path = os.path.join("/scratch/gilbreth/thr/dev/RUM/datasets/tiny-imagenet-200/val")
        self.test_path = os.path.join("/scratch/gilbreth/thr/dev/RUM/datasets/tiny-imagenet-200/test")
