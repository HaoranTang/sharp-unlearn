...
def warmup_lr(epoch, step, optimizer, one_epoch_step, args):
    # Initial lr is 1% of target lr
    initial_lr = args.lr * 0.01
    overall_steps = args.warmup * one_epoch_step
    current_steps = epoch * one_epoch_step + step

    lr = initial_lr + (args.lr - initial_lr) * current_steps / overall_steps
    lr = min(lr, args.lr)
...
def dataset_convert_to_test(dataset, args=None):
    if args.dataset == "TinyImagenet":
        ...
        print("Converting train subsets to test transforms")
    ...
    if args.dataset == 'imagenet' or args.dataset == 'tiny_imagenet':
        dataset.set_transform(transform=test_transform)
    else:
...
def setup_model_dataset(args):
    if args.dataset == "cifar10":
        ...
        train_full_loader, val_loader, train_idx = cifar100_dataloaders_idx(
            batch_size=args.batch_size, data_dir=args.data, num_workers=args.workers,
            no_aug=args.no_aug, corruptions=args.corrupt
        )
        ...
        model = model_dict[args.arch](num_classes=1000, imagenet=True, pretrained=True)
        in_feats = model.fc.in_features
        model.fc = nn.Linear(in_feats, classes)

        model.normalize = normalization
        print(model)
        if args.class_to_replace is None and args.num_indexes_to_replace is None:
            loaders = prepare_data(dataset="tiny_imagenet", batch_size=args.batch_size, data_path=args.data)
            train_loader, val_loader = loaders["train"], loaders["val"]
            return model, train_loader, val_loader
            ...
            val_loader = loaders["val"]
            return model, retain_loader, forget_loader, val_loader
        elif args.num_indexes_to_replace is not None:
            # (a) first get full train+val so we know dataset sizes
            full = prepare_data(
                dataset="tiny_imagenet",
                batch_size=args.batch_size,
                data_path=args.data,
            )
            full_train_ds = full["train"].dataset
            N = len(full_train_ds)
            # (b) build a mask of size N, drop at random
            train_subset_indices = torch.ones(N)
            k = min(args.num_indexes_to_replace, N)
            drop_idx = torch.randperm(N)[:k]
            train_subset_indices[drop_idx] = 0
            # (c) re‐prepare with that train mask
            loaders = prepare_data(
                dataset="tiny_imagenet",
                batch_size=args.batch_size,
                train_subset_indices=train_subset_indices,
...
def get_loader_from_dataset(dataset, batch_size, seed=1, shuffle=True):
    return torch.utils.data.DataLoader(
        dataset, batch_size=batch_size, num_workers=12, pin_memory=True, shuffle=shuffle
    )
